...
function imsources_getModuleAdminLink($moduleName='imsources') {
	global $icmsModule;
	...
	$ret = '';
	if ($moduleName) {
		$ret = "<a href='" . ICMS_URL . "/modules/$moduleName/admin/index.php'>" ._MD_IMSOURCES_ADMIN_PAGE . "</a>";
	}
	return $ret;
}
 ...
 * @todo to be move in icms core
 */
function imsources_getModuleName($withLink = true, $forBreadCrumb = false, $moduleName = false) {
	if (!$moduleName) {
		global $icmsModule;
		$moduleName = $icmsModule->getVar('dirname');
	}
 ...
 */
function imsources_getPreviousPage($default=false) {
	global $impresscms;
 ...
 */
function imsources_getMonthNameById($month_id) {
...
*/
function imsources_getLinkedUnameFromId($userid, $name = false, $users = array (), $withContact = false)
{
